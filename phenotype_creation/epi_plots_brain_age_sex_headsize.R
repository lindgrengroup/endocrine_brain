...
ids <- fread("/mnt/project/data/brain_all/genotype_process/all/dosage/assoc.resid_HT-10th_percentile-mod=FLAIR-threshold=0.3-warpResolution=2mm-norm_norm.regenie.merged.withX.filtered.merged_incl_samples", data.table=FALSE)

head_size <- fread("/mnt/project/data/data_participant.csv", data.table=FALSE)
...
ids$ID_1 <- as.character(ids$ID_1)
head_size$eid <- as.character(head_size$eid)
...
volume_age_sex_clean <- subset(volume_age_sex, eid_11867 %in% ids$ID_1)

head_size <- head_size[,c("eid", "p26521_i2")]
head_size_age_sex <- merge(head_size, sex_age, by="eid")
head_size_age_sex_clean <- subset(head_size_age_sex, eid %in% ids$ID_1)

age_mean <- mean(volume_age_sex_clean$p21003_i2)
#63.74846
age_sd <- sd(volume_age_sex_clean$p21003_i2)
#7.496477

percentage_female <- sum(volume_age_sex_clean$p22001=="Female")/length(volume_age_sex_clean$p22001)*100
#53.07171

vol_corr_input <- volume_age_sex_clean[,c("HT-volume-threshold=0.3-warpResolution=2mm", "PG-volume-threshold=0.3-warpResolution=2mm", "HT-SumGM-threshold=0.3-warpResolution=2mm",
                                          "OB_LR-volume-threshold=0.3-warpResolution=2mm")]

vol_corr_output <- rcorr(as.matrix(vol_corr_input), type="spearman")
vol_corr_output_round <- round(vol_corr_output$r, digits = 2)
write.csv(vol_corr_output_round, "vol_corr_table.csv", quote=FALSE) 
system("dx upload vol_corr_table.csv --path data/")
...
volume_age_sex_long_clean <- volume_age_sex_long %>% mutate(age_bin=cut(p21003_i2, breaks=c(45,50,55,60,65,70,75,80,85), include.lowest=TRUE))
...
volume_age_sex_clean<- volume_age_sex_clean %>% mutate(age_bin=cut(p21003_i2, breaks=c(45,55,65,75,85), include.lowest = TRUE))

age_mean_volume <- aggregate(volume_age_sex_clean[,c("HT-volume-threshold=0.3-warpResolution=2mm", "PG-volume-threshold=0.3-warpResolution=2mm",
...
sex_mean_volume <- aggregate(volume_age_sex_clean[,c("HT-volume-threshold=0.3-warpResolution=2mm", "PG-volume-threshold=0.3-warpResolution=2mm",
                                              "OB_LR-volume-threshold=0.3-warpResolution=2mm", "HT-SumGM-threshold=0.3-warpResolution=2mm")],
                      list(volume_age_sex_clean$p22001), FUN=function(x) c(mean=mean(x), sd=sd(x)))

head_size_age_sex_clean$p21003_i2 <- as.numeric(head_size_age_sex_clean$p21003_i2)
head_size_age_sex_clean <- head_size_age_sex_clean[complete.cases(head_size_age_sex_clean),]
head_size_age_sex_clean <- head_size_age_sex_clean %>% mutate(age_bin=cut(p21003_i2, breaks=c(45,55,65,75,85), include.lowest = TRUE))

options(digits=8)
age_mean_head_size <- aggregate(head_size_age_sex_clean[,c("p26521_i2")],
                             list(head_size_age_sex_clean$age_bin), FUN=function(x) c(mean=mean(x), sd=sd(x)))
sex_mean_head_size <- aggregate(head_size_age_sex_clean[,c("p26521_i2")],
                             list(head_size_age_sex_clean$p22001), FUN=function(x) c(mean=mean(x), sd=sd(x)))

mean_head_size_all <- mean(head_size_age_sex_clean$p26521_i2)
sd_head_size_all <- sd(head_size_age_sex_clean$p26521_i2)
#THIS NEEDS FIXED
all_mean <- aggregate(
  volume_age_sex_long[,"value"], 
  by = list(measure = volume_age_sex_long$measure), 
...
write.csv(volume_age_sex_clean_long_sum, "epi_brain_age_sex_mean_all_revised.csv", quote=FALSE, row.names=FALSE)
system("dx upload epi_brain_age_sex_mean_all_revised.csv --path data/")
...
age_names <- list(
  "[45,50]"="45-50", "(50,55]"="51-55", "(55-60]"="56-60",
  "(60-65]"="61-65", "(65,70]"="66-70", "(70-75]"="71-75",
  "(75-80]"="76-80", "(80,85]"="81-85"
)
...
pdf("epi_brain_age_sex_updated.pdf", width=8, height=4.8)
p
dev.off()
