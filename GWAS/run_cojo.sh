...
pheno_list_a=($(echo ${pheno_list} | tr " " "\n"))
for item in ${pheno_list_a[@]}; do
file=assoc.$item.regenie.merged.filtered.cojo_format.ma
...
for item in ${pheno_list_a[@]}; do
file=assoc.$item.regenie.merged.filtered.cojo_format.ma
	pheno_name="$(basename "$file" .cojo_format.ma)"
	merge_cmd='out_name="'"$(basename "$file" .cojo_format.ma).merged.txt"'"
	cp '"/mnt/project/${project_out}/genotype_process/${pheno_group}/regenie_step2/filtered/cojo_results/${pheno_name}_c*_cojo_result.jma.cojo"' .
	...
	rm *jma.cojo'
	dx run swiss-army-knife -iin="${project_out}/genotype_process/${pheno_group}/regenie_step2/filtered/cojo_results/${pheno_name}_c1_cojo_result.log" \
